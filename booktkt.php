...
$sql1 = "SELECT PNR FROM tickets ORDER BY rand() LIMIT 1";
$result1 = mysqli_query($conn, $sql1);
$row1 = mysqli_fetch_assoc($result1);

	if(mysqli_query($conn, $query))
{  
	echo "<script type='text/javascript'>alert('Your PNR is ' +'$row1[PNR]');</script>";
...
<body>
	...
	<form method="post" name="journeyform" onsubmit="return validate()">
		<select id="trains" name="trains" required>
			...
			<option value="Mysoreexp" >Mysore Express - Talguppa to Mysore</option>
			<option value="Tirupatiexp" >Tirupati Express - Bengaluru to Tirupati</option>
			<option value="Renigunta weeklyexp">Renigunta Express - Mysore to Renigunta</option>
			<option value="Chennaiexp" >Chennai express - Chennai to Mumbai</option>
			<option value="Mumbaiexp" >Mumbai Express - Mumbai to KSR</option>
		</select>
		<br/><br/>
